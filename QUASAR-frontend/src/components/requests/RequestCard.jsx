    ...
    <motion.div variants={itemVar} layout>
      <Card className={`group relative border-none shadow-sm hover:shadow-xl transition-all duration-500 rounded-[32px] overflow-hidden bg-white ${item.status !== 'PENDING' ? 'opacity-75 grayscale-[0.5]' : ''}`}>
        ...
        <CardContent className="p-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <Link to={`/projects/${item.project?.id}`} className="block text-xl font-black text-slate-900 leading-tight hover:text-blue-600 transition-colors">
                  {item.project?.title}
                </Link>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter">
                  {isInvitation ? 'Invited by' : 'Request from'} <span className="text-slate-600 underline decoration-blue-200 underline-offset-4 font-black">{relevantUser?.firstName} {relevantUser?.lastName}</span>
                </p>
              </div>
            </div>
          ...
          {/* Message Bubble */}
          {item.message && (
            <div className="mt-6 p-5 bg-blue-50/50 rounded-[24px] rounded-tl-none border border-blue-100 relative group-hover:bg-blue-50 transition-colors">
              <MessageSquare size={14} className="absolute -top-2 -left-2 text-blue-400 bg-white rounded-full p-0.5" />
          ...
          {item.status === 'PENDING' && (
            <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4">
              <div className="flex gap-3 flex-1 md:flex-none">
                {onAccept && (
                  <Button
                    ...
                    className="flex-1 md:flex-none h-12 px-8 rounded-2xl bg-blue-600 hover:bg-blue-700 font-black shadow-lg shadow-blue-100"
                  >
              ...
              <Button variant="ghost" className="text-xs font-black text-slate-400 uppercase tracking-widest hover:text-blue-600 group/btn" asChild>
