...
class ProjectService {
  ...
  async getProjectsByCollege(collegeId) {
    try {
      return await apiService.get(`/projectDiscovery/by-college/${collegeId}`);
    } catch (error) {
  ...
  async searchProjects(searchParams = {}) {
    try {
      ...
      const endpoint = queryString
        ? `/projectDiscovery/searchProjects?${queryString}`
        : '/projectDiscovery/searchProjects';
  ...
  async discoverProjects(page = 0, size = 8) {
    try {
      const params = { page, size };
      return await apiService.get('/projectDiscovery/discover', params);
    } catch (error) {
