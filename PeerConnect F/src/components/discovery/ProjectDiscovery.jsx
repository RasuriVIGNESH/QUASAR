  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col font-sans selection:bg-blue-100 dark:selection:bg-blue-900">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] rounded-[32px] p-0 border-none overflow-hidden shadow-2xl flex flex-col">
          <AnimatePresence>
            {selectedProject && (
              <div className="flex flex-col w-full h-full overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="bg-slate-900 p-10 text-white relative overflow-hidden shrink-0">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-blue-600/10 rounded-full -mr-32 -mt-32 blur-3xl" />
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                      <Badge className="bg-blue-600 text-white border-none font-bold px-3">{selectedProject.categoryName}</Badge>
                      <Badge variant="outline" className="text-slate-400 border-slate-700 font-bold px-3 uppercase tracking-widest text-[10px]">{selectedProject.status}</Badge>
                    </div>
                    <h2 className="text-4xl font-black mb-4 tracking-tight">{selectedProject.title}</h2>
                    <div className="flex items-center gap-6 text-slate-400 text-xs font-bold uppercase tracking-widest">
                      <span className="flex items-center gap-2"><Users className="h-4 w-4" /> {selectedProject.currentTeamSize} / {selectedProject.maxTeamSize} Capacity</span>
                      <span className="flex items-center gap-2"><Clock className="h-4 w-4" /> Posted recently</span>
                    </div>
                  </div>
                </div>

                <div className="p-10 bg-white overflow-y-auto scrollbar-hide flex-1 min-h-0">
                  <div className="space-y-8">
                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Objective & Scope</h4>
                      <p className="text-slate-600 text-lg leading-relaxed font-medium">
                        {selectedProject.description || selectedProject.goals || 'The team has not yet provided a detailed description for this objective.'}
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedProject.requiredSkills?.map((s, i) => (
                          <Badge key={i} className="bg-slate-50 border-slate-200 text-slate-700 px-4 py-1.5 rounded-xl text-sm font-bold shadow-sm">{s}</Badge>
                        ))}
                      </div>
                    </div>
                      ...
                      {!sentRequests.has(selectedProject.id) && (
                        <>
                          ...
                          <Textarea
                            ...
                            className="rounded-2xl border-slate-200 min-h-[120px] p-5 text-base focus:ring-2 focus:ring-blue-500 shadow-inner"
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4 justify-start shrink-0">
                  <Button
                    onClick={handleJoinRequest}
                    disabled={isSubmitting || sentRequests.has(selectedProject.id)}
                    className={`rounded-xl px-10 h-12 font-black transition-all shadow-lg hover:scale-105 ${sentRequests.has(selectedProject.id) ? 'bg-emerald-500 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-100'}`}
                  >
                    {isSubmitting ? (
                      <Loader2 className="animate-spin h-5 w-5" />
                    ) : sentRequests.has(selectedProject.id) ? (
                      <span className="flex items-center gap-2"><Send className="h-4 w-4" /> Application Sent</span>
                    ) : (
                      <span className="flex items-center gap-2">Submit Application <ChevronRight className="h-4 w-4" /></span>
                    )}
                  </Button>
                </div>

                {/* AI Chat Bot Assistant */}
                <ProjectChatBot project={selectedProject} />
              </div>
            )}
          </AnimatePresence>
        </DialogContent>
