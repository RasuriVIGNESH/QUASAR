    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-8 pt-10">
        ...
        >
          {loading ? (
            [...Array(8)].map((_, i) => (
              <Card key={i} className="h-40 border-none shadow-sm rounded-[32px] bg-white p-6">
                <div className="flex flex-col h-full">
                  <div className="flex justify-between mb-4">
                    <Skeleton className="w-10 h-10 rounded-2xl bg-slate-100" />
                    <Skeleton className="w-12 h-4 rounded bg-slate-100" />
                  </div>
                  <Skeleton className="w-3/4 h-6 mb-2 rounded bg-slate-100" />
